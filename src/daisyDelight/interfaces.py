...
class TkGUI(Tkinter.Frame):
	def __init__(self,state,master=None):
		...
		import tkFileDialog
		self.filedialog=tkFileDialog.Open(filetypes=(("Ncc File",".html",".htm"), ))
	...
	def updateTOC(self, (name, id,level)):
		...
		for n, i, l in zip (name, id,level):
			if(l>levels[-1]):#make sub menue
				...
				menus[-2].add_cascade(label=lvl[l],menu=menus[-1],command=lambda x=i:self.state.setId(x))
				levels.append(l)
			...
			menus[-1].add_command(label =n, command = lambda x=i: self.state.setId(x))
			#print n,l,levels
	...
	def createwidgets(self):
		self.book_label=Tkinter.Label(self,text="...", width =40, font = ("Monaco",14), wraplength = 300)
		self.book_label.grid(column=3,row=0,columnspan = 7, sticky="we")
		self.chapter_label=Tkinter.Label(self,text="...", width = 40, wraplength = 300, font =("Monaco",14))
		self.chapter_label.grid(column=2,row=2,columnspan=7,rowspan = 3, sticky="we")
		self.msg_label=Tkinter.Label(self,text="Welcome to Daisy Delight...", font = ("Monaco",14), wraplength = 300, width = 40)
		self.msg_label.grid(column=2,row=6,columnspan=7,sticky="we")
		self.load_button=Tkinter.Button(self,text="Load",command=self.load_command)
		self.load_button.grid(column=0,row=0,columnspan=1,sticky="we")
		self.stop_button=Tkinter.Button(self,text="Stop",command=self.stop_command, state = 'disabled')
		self.stop_button.grid(column=1,row=3,columnspan=1,sticky="we")
		self.prew_button=Tkinter.Button(self,text="Prew",command=self.state.prew, state='disabled')
		self.prew_button.grid(column=0,row=2,columnspan=1)
		self.navigateButtons.append(self.prew_button)
		self.next_button=Tkinter.Button(self,text="Next",command=self.state.next, state='disabled')
		self.next_button.grid(column=1,row=2,columnspan=1)
		self.navigateButtons.append(self.next_button)
		self.down_button=Tkinter.Button(self,text="down",command=self.state.levelDown, state = 'disabled')
		self.down_button.grid(column=0,row=1,columnspan=1)
		self.navigateButtons.append(self.down_button)
		self.up_button=Tkinter.Button(self,text="up",command=self.state.levelUp, state = 'disabled')
		...
		self.skip_next_button=Tkinter.Button(self,text="skip",state = 'disabled', command=self.state.skip)
		self.skip_next_button.grid(column=0,row=4,columnspan=2,sticky="we")
		self.navigateButtons.append(self.skip_next_button)
		self.play_button=Tkinter.Button(self,text="Play",command=self.play_command, state='disabled')
		self.play_button.grid(column=0,row=3,columnspan=1,sticky="we")
		self.quit_button=Tkinter.Button(self,text="Quit",command=self.quit_command)
		self.quit_button.grid(column=1,row=0,columnspan=1,sticky="we")
		self.toc=Tkinter.Menubutton(self)#OptionMenu(self,self.toc_var,"TOC")
		self.toc["text"]="TOC"
		self.toc.grid(column=0,row=6,columnspan=2,sticky="e")
		self.tocmenu=Tkinter.Menu(self.toc)
		self.toc['menu']=self.tocmenu

		modes = [('h1',0),('h2',1),('h3',2),('h4',3),('h5',4),('h6',5),('pg',6) ]
		self.level_radio = []
		for  text, value in modes:
			self.level_radio.append( Tkinter.Radiobutton(self, text = text, variable = self.radio_level, value = value, command = self.set_level))
			self.level_radio[-1].grid(column=(value+2) ,row=1,columnspan=1,sticky="we")
