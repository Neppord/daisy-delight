...
class XmlReader:
    ...
    def startElemetHandler(self, name, attrs):
        if "start_" + name in self.__class__.__dict__ and callable(
            self.__class__.__dict__["start_" + name]):
            self.__class__.__dict__["start_" + name](self, attrs)
        else:
            self.start_default(name, attrs)

    def endElementHandler(self, name):
        if "end_" + name in self.__class__.__dict__ and callable(
            self.__class__.__dict__["end_" + name]):
...
class NccParser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if tag.lower() in levels:
            try:
                nextPrew = getDirectionDict(self.chron[-1]['level'],
                                            self.chron[-1]['prew'],
                                            self.chron[-1]["id"])
            except KeyError:
    ...
    def getTOC(self):
        ...
        for now, prew in zip(self.chron[-2:0:-1], self.chron[-3::-1]):
            prew['next'] = getDirectionDict(now['level'], now['next'],
                                            now['id'])
